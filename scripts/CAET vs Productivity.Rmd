---
title: "CAET vs Productivity"
output: html_document
date: "2024-02-21"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
    echo = FALSE,
    warning = FALSE,
    error = FALSE,
    message = FALSE
)

#renv::restore()

# Load Libraries
library(tidyverse)
library(openxlsx)
library(flextable)
library(fmsb)

#read in performance data
Main_Survey <- read.xlsx("FILEPATH TO DATA HERE",
                        sheet = "Main Survey",
                        check.names = TRUE)

Indicators <- read.xlsx("FILEPATH TO DATA HERE",
                        sheet = "Calculated Indicators",
                        check.names = TRUE)%>%
  select(farm_id, dietary_diversity:pest_score)%>%
  select(-c(inc3,structure, compaction, depth, residues,
            color, water_ret, cover, erosion, invertebrates, microbio))

data <- left_join(Main_Survey,
                         Indicators)

#import functions to use for analysis
source("../functions.R")
```

## Value of crop and forestry products produced

### Crop Value

#### Overall Scatter

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "crop_prodval",
    ytitle = "Total value of crop production (Kenyan Shilling)",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### By Region

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "crop_prodval",
    facet = "location_level_one_name",
    ytitle = "Total value of crop production (Kenyan Shilling)",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### Spearman's rank Correlations

```{r}
correlations_table(indicator = "crop_prodval")
```

#### Test of Mean - ProSoils vs Comparison

```{r}
mean_test(indicator = "crop_prodval",
          title = "Total value of crop production (Kenyan Shilling)")
```

#### Test of Median - ProSoils vs Comparison

```{r}
median_test(indicator = "crop_prodval",
          title = "Total value of crop production (Kenyan Shilling)")
```

### Total value of crop and forestry products

#### Overall Scatter

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "cfp_prodval",
    ytitle = "Total value of crop and forestry products (Kenyan Shilling)",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### By Region

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "cfp_prodval",
    facet = "location_level_one_name",
    ytitle = "Total value of crop and forestry products (Kenyan Shilling)",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### Spearman's rank Correlations

```{r}
correlations_table(indicator = "cfp_prodval")
```

#### Test of Mean - ProSoils vs Comparison

```{r}
mean_test(indicator = "cfp_prodval",
          title = "Total value of crop and forestry products (Kenyan Shilling)" )
```

#### Test of Median - ProSoils vs Comparison

```{r}
median_test(indicator = "cfp_prodval",
          title = "Total value of crop and forestry products (Kenyan Shilling)")
```

## Value of animal and livestock products produced

### animal Value

#### Overall Scatter

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "anim_prodval",
    ytitle = "Total value of animal production (Kenyan Shilling)",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### By Region

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "anim_prodval",
    facet = "location_level_one_name",
    ytitle = "Total value of animal production (Kenyan Shilling)",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### Spearman's rank Correlations

```{r}
correlations_table(indicator = "anim_prodval")
```

#### Test of Mean - ProSoils vs Comparison

```{r}
mean_test(indicator = "anim_prodval",
          title = "Total value of animal production (Kenyan Shilling)")
```

#### Test of Median - ProSoils vs Comparison

```{r}
median_test(indicator = "anim_prodval",
          title = "Total value of animal production (Kenyan Shilling)")
```

### Total value of animal and livestock products

#### Overall Scatter

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "anpr_prodval",
    ytitle = "Total value of animal and livestock products (Kenyan Shilling)",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### By Region

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "anpr_prodval",
    facet = "location_level_one_name",
    ytitle = "Total value of animal and livestock products (Kenyan Shilling)",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### Spearman's rank Correlations

```{r}
correlations_table(indicator = "anpr_prodval")
```

#### Test of Mean - ProSoils vs Comparison

```{r}
mean_test(indicator = "anpr_prodval",
          title = "Total value of animal and livestock products (Kenyan Shilling)" )
```

#### Test of Median - ProSoils vs Comparison

```{r}
median_test(indicator = "anpr_prodval",
          title = "Total value of animal and livestock products (Kenyan Shilling)")
```

## Total value of farm outputs (crops, animals, crop and forestry products, animal products)

### Total Output

#### Overall Scatter

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "total_output",
    ytitle = "Total value of farm outputs\n(crops, animals, crop and forestry products, animal products)\n
    Kenyan Shilling",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### By Region

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "total_output",
    facet = "location_level_one_name",
    ytitle = "Total value of farm outputs\n(crops, animals, crop and forestry products, animal products)\n
    Kenyan Shilling",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### Spearman's rank Correlations

```{r}
correlations_table(indicator = "total_output")
```

#### Test of Mean - ProSoils vs Comparison

```{r}
mean_test(indicator = "total_output",
          title = "Total value of farm outputs\n(crops, animals, crop and forestry products, animal products)\n
    Kenyan Shilling")
```

#### Test of Median - ProSoils vs Comparison

```{r}
median_test(indicator = "total_output",
          title = "Total value of farm outputs\n(crops, animals, crop and forestry products, animal products)\n
    Kenyan Shilling")
```

## Gross value of the agricultural production (per person / per ha)

### Per person

#### Overall Scatter

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "tot_productivity_pers",
    ytitle = "Total productivity per person\ncrops, animals, crop\nand forestry products, animal products",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### By Region

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "tot_productivity_pers",
    facet = "location_level_one_name",
    ytitle = "Total productivity per person\ncrops, animals, crop\nand forestry products, animal products",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### Spearman's rank Correlations

```{r}
correlations_table(indicator = "tot_productivity_pers")
```

#### Test of Mean - ProSoils vs Comparison

```{r}
mean_test(indicator = "tot_productivity_pers",
          title = "Total productivity per person\ncrops, animals, crop\nand forestry products, animal products")
```

#### Test of Median - ProSoils vs Comparison

```{r}
median_test(indicator = "tot_productivity_pers",
          title = "Total productivity per person\ncrops, animals, crop\nand forestry products, animal products")
```

### Per ha

#### Overall Scatter

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "tot_productivity_ha",
    ytitle = "Total productivity per ha\ncrops, animals, crop\nand forestry products, animal products",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### By Region

```{r}
caet_performance_scatter(
    x = "totscore_caet",
    y = "tot_productivity_ha",
    facet = "location_level_one_name",
    ytitle = "Total productivity per ha\ncrops, animals, crop\nand forestry products, animal products",
    xtitle = "Total CAET Score",
    scale = "log10"
)
```

#### Spearman's rank Correlations

```{r}
correlations_table(indicator = "tot_productivity_ha")
```

#### Test of Mean - ProSoils vs Comparison

```{r}
mean_test(indicator = "tot_productivity_ha",
          title = "Total productivity per ha\ncrops, animals, crop\nand forestry products, animal products")
```

#### Test of Median - ProSoils vs Comparison

```{r}
median_test(indicator = "tot_productivity_ha",
          title = "Total productivity per ha\ncrops, animals, crop\nand forestry products, animal products")
```